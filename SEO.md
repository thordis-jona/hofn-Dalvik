...
- Corpus endpoints at `/schema/site.jsonld` and `/schema/guide.jsonld` (both return `application/ld+json`).
...
- `/schema/site.jsonld`, `/schema/guide.jsonld`, `/schemamap.xml`, and `/.well-known/api-catalog` expose the structured corpus.
