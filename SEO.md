...
- Corpus endpoints at `/schema/site.json` and `/schema/guide.json` (both return `application/ld+json`).
...
- `/schema/site.json`, `/schema/guide.json`, `/schemamap.xml`, and `/.well-known/api-catalog` expose the structured corpus.
