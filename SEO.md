...
- Corpus endpoints at `/.well-known/schema-site` and `/.well-known/schema-guide` (both return `application/ld+json`).
...
- `/.well-known/schema-site`, `/.well-known/schema-guide`, `/schemamap.xml`, and `/.well-known/api-catalog` expose the structured corpus.
